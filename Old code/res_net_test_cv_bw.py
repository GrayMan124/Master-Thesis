...
class TopoBlockIN(nn.Module):
    def __init__(self, in_channels, out_channels, identity_downsample=None, topo_size_in = 0, stride=1):
        super(Block, self).__init__()
...
if __name__ == "__main__":
    ...
    # raise("tego")
    data_fin = process_data_topo(trainset)

    # trainset.data = data_fin
    # raise("Jest mozliwe")
    trainset = MyDataset(data_fin,trainset.targets)
    ...
    data_fin = process_data_topo(testset)
    testset = MyDataset(data_fin,testset.targets)
    # testset.data = data_fin
    # testset.data = torch.stack([data_fin])
