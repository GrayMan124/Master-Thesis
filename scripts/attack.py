...
from benchmarks.benchamrk_utils import (
    ModelWrapper,
    run_auto_attack,
    CleanImageDatasetLoader,
)
...
if __name__ == "__main__":
    ...
    test_ds = PrecomputedDataset(
        data_path, version_folders=["test"], transform=resize_transform
    )
    ...
    state_dict = torch.load(
        os.path.join(args.modelPath, f"{args.name}.pkl"), weights_only=True
    )
    ...
    run_auto_attack(
        model=wrapped_model, x_test=x_test, y_test=y_test, log_path="./logs/"
    )
